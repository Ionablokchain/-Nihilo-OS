...
class Parser:
    ...
    def _declaration(self) -> Node:
        ...
        raise ParseError(
            f"unexpected token {self._peek().value!r} at line {self._peek().line}: "
            f"expected a top-level declaration"
        )
    ...
    def _parse_intention(self) -> IntentionDecl:
        ...
        while not self._check(TokenType.RBRACE):
            ...
            if t == TokenType.TRIGGER:
                ...
                # Accept literals and unary-minus literals; folding here keeps
                # the semantic analyzer's range check meaningful.
                if isinstance(expr, NumberLiteral):
                    ...
                    raise ParseError(
                        f"priority must be a numeric literal at line {start.line}"
                    )
                ...
                raise ParseError(
                    f"unexpected token {self._peek().value!r} in intention body "
                    f"at line {self._peek().line}"
                )
    ...
    def _parse_collapse_core(self) -> Collapse:
        ...
        if self._check(TokenType.IDENT):
            ...
            raise ParseError(
                f"expected collapse method name at line {self._peek().line}"
            )
        self._consume(TokenType.RPAREN)
    ...
    def _primary(self) -> Node:
        ...
        raise ParseError(
            f"unexpected token {tok.value!r} at line {tok.line}, column {tok.column}"
        )
    ...
    def _parse_dist_literal(self) -> "DistLiteral":
        ...
        if self._check(TokenType.RBRACE):
            raise ParseError(
                f"empty dist literal at line {self._peek().line} "
                f"(write at least one value:weight pair)"
            )
    ...
    def _consume(self, t: TokenType) -> Token:
        ...
        cur = self._peek()
        raise ParseError(
            f"expected {t.name}, got {cur.type.name} ({cur.value!r}) "
            f"at line {cur.line}, column {cur.column}"
        )
